			...
			while(iter.hasNext()){
				...
				if(nodeType.equalsIgnoreCase("FOG_DEVICE")){
					...
					try {					
						...
						sensorPriority = Integer.parseInt(node.get("priority").toString()); // Modificado para priorizar processamento
					} catch (NullPointerException e) {
						// prioridade não definida, setando prioridade padrão 						
						sensorPriority = 0;						
					}
					...
					sensors.add(new Sensor(nodeName, sensorType, userId, appId, distribution, sensorPriority));
