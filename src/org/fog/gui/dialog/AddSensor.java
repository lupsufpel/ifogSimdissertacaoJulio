...
public class AddSensor extends JDialog {
	...
	private JTextField sensorPriority;
...
//				else if(sensorPriority.getText().length() < 0) {
//					prompt("invalid priority","Error");
//				}
				else {
					...
					String priority = sensorPriority.getText();
					if(dist.equals("Normal")){
						...
						if(!catchedError){//adicionar o Priority class
							SensorGui sensor = new SensorGui(sensorName.getText().toString(), _sensorType, (String)distribution.getSelectedItem(),
											normalMean_, normalStdDev_, uniformLow_, uniformUp_, deterministicVal_, priority);
	...
	private JPanel createInputPanelArea() {
		...
		springPanel.add(lPriority);
		sensorPriority = new JTextField();
		lPriority.setLabelFor(sensorPriority);
		springPanel.add(sensorPriority);
